...
public class KSPayWebHostBean
{
	...
	static String	LOG_HOME_DIR		= Constants.KSPAY_LOG ;//로그를 남길경우 LOG_HOME_DIR을 "c:/log/" 와 같이 '/'를 경로 구분자로 하여 '/'로 끝나도록 로그를 남길 폴더를 지정해야 함
	...
	static String KSPAY_MOBILEHOST_URL = "http://kspay.ksnet.to/store/KSPayMobileV1.4/web_host/recv_post.jsp";
	...
	public boolean kspay_send_msg(String _mtype, String type)
	{
		this.mtype = _mtype;
		String rmsg = send_url(type);
		if (rmsg.indexOf('`') != -1)
		{
    ...
    public String send_url(String type)
    {
		...
		try {
			...
			if(type.equals("1")) {
				...
				httpConn = new URL(KSPAY_MOBILEHOST_URL).openConnection();
			}

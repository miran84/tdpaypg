...
//			person_email3: { required : "필수로입력하세요"},
			cpid: { alphanumeric : "영문과 숫자만 입력하세요"},
...
//    	return this.optional(element) ||  /^.*(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/.test(value);
	...
	$("#corp_type").change(function(){
	    ...
	    if(corp_type == "A"){
	    	...
	    	$("#corp_regist_num2").attr('placeholder','주민등록번호');
	    } else if(corp_type == "B"){
	    	$("#corp_regist_num2").show();
	    	$("#corp_regist_num2").attr('placeholder','법인등록번호');
	    } else {
	    	$("#corp_regist_num2").hide();
	    	$("#corp_regist_num2").attr('placeholder','');
	    }
    })

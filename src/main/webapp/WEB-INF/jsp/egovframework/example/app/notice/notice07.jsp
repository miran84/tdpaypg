				...
				<c:if test="${result.high_menu_id eq '6000'}">
					<c:choose>
						<c:when test="${result.menu_id eq '6001' and sessionScope.role_id eq '1001'}">
			                <option selected value="<c:out value="${result.menu_url}" />"><c:out value="${result.menu_nm}" />(${sessionScope.CANCEL_NUM})</option>
						</c:when>
...
<script src="/app/js/notice07.js"></script>

<section class="notion_basic">
    <div class="inner">
        <div class="ttl">
            <h2 class="ttl_left">결제취소요청</h2>
			...
			<input type="hidden" name="store_id" id="store_id" value="<c:out value="${sessionScope.login_id}" />">
                ...
                <fieldset>
                    ...
                    <div class="search_btn">
                        <button onclick="notice07_data();" class="btn-active" type="button">검색</button>
                    </div>
                </fieldset>
            </form>
        </div>
